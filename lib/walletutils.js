...
WalletUtils.signTxp = function(txp, xPrivKey) {
  ...
  $.checkArgument(txp.toAddress && Bitcore.Address.isValid(txp.toAddress), 'toAddress is invalid');
  $.checkArgument(txp.amount && _.isNumber(txp.amount) && txp.amount > 0, 'amount is invalid');
  $.checkArgument(txp.changeAddress && txp.changeAddress.address && Bitcore.Address.isValid(txp.changeAddress.address), 'changeAddress is invalid');
  $.checkArgument(txp.inputs && _.isArray(txp.inputs) && txp.inputs.length > 0, 'The transaction must have at least one input');
  $.checkArgument(txp.requiredSignatures && _.isNumber(txp.requiredSignatures) && txp.requiredSignatures > 0, 'requiredSignatures is invalid');

  $.checkArgument(Bitcore.Address(txp.toAddress).network.name === Bitcore.Address(txp.changeAddress.address).network.name, 'toAddress and changeAddress must be on the same network');
